...
public class ProxyBoxLayoutTest {
    ...
    @ParameterizedTest
    @CsvSource(
            {"left top,2,2,1,1,0,1", "right top,2,2,1,2,0,2", "left bottom,2,2,2,1,3,1", "right bottom,2,2,2,2,3,2",
             "left mid,4,4,2,2,0,2", "right mid,4,4,2,3,0,3", "left middle,4,4,3,2,5,2", "right middle,4,4,3,3,5,3",
    ...
    void testFindEmptyCell(String test, int height, int width, int appRow, int appCol, int proxyRow, int proxyCol) {
        ...
        var proxyBoxLayout = new ProxyBoxLayout(comp);
        // test
        var result = proxyBoxLayout.findNearestEmptyCell(innerAppCoord);
        // verify
